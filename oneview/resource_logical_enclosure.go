...
func resourceLogicalEnclosureCreate(d *schema.ResourceData, meta interface{}) error {
	...
	//enclosureUriList := d.Get("enclosure_uris").(*schema.Set).List()
	...
	for i:=0; i< enclosureSetCount; i++{
		enclosureSetPrefix := fmt.Sprintf("enclosure_uri.%d",i)
		if val, ok := d.GetOk(enclosureSetPrefix);ok{
			enclosureUris[i] = utils.NewNstring(val.(string))
		}
	}
	//enclosureSorted := sort.Strings(enclosureUriList[:])
	//enclosureUris := make([]utils.Nstring, len(enclosureUriList))
	//enclosureUris := make(map[int]utils.Nstring, len(enclosureUriList))
	//for i, raw := range enclosureUriList {
	//	enclosureUris[i] = utils.NewNstring(raw.(string))
	//}
	...
	//firmwares := make([]ov.LogicalEnclosureFirmware, 0)
	...
	//logicalEnclosure.firmwares = firmwares
...
func resourceLogicalEnclosureRead(d *schema.ResourceData, meta interface{}) error {
	...
	if err != nil || logicalEnclosure.URI.IsNil() {
		d.SetId("")
		return nil
	}
	d.SetId("name")
