...
func resourceHypervisorClusterProfileCreate(d * schema.ResourceData, meta interface {}) error {
    ...
    for i,
    rawadd: = range rawAddHostRequests {
    ...
    for i,
    rawshsvol: = range rawSharedStorageVolumes {
    ...
    for _,
    raw: = range HypervisorClusterSettingslist {
    ...
    var virtualSwitchesCollect[] ov.VirtualSwitches
        /*****************************virtual switch********************************/
    if val,
    ok: = d.GetOk("virtual_switches");ok {
        ...
        for _,
        rawvs: = range rawVirtualSwitches {
            ...
            /***************virtual switch uplinks start***************************/
            {
                ...
                if rawVirtualSwitchesItem["virtual_switch_uplinks"] != nil {
                        ...
                        for _,
                        rawvsul: = range rawVirtualSwitchUplinks {
                ...
                for _, rawvspg: = range rawVirtualSwitchPortGroups {
                        ...
                        for i,
                        rawvspgnw: = range rawNetworkUris {
                        ...
                        for _,
                        rawvsp: = range rawVirtualSwitchPorts {
                                ...
                                for i,
                                rawpp: = range rawVirtualPortPurpose {
    ...
    for _,
    rawscp: = range rawVirtualSwitchConfigPolicy {
    ...
    for _,
    rawhcp: = range rawHostConfigPolicy {
    ...
    hypervisorProfileTemplate: = ov.HypervisorHostProfileTemplate {}
        ...
        for _,
    rawht: = range rawHypervisorHostProfileTemplate {
...
func resourceHypervisorClusterProfileRead(d * schema.ResourceData, meta interface {}) error {
    config: = meta.( * Config)
    hypCP,
    err: = config.ovClient.GetHypervisorClusterProfileByName(d.Id())
    ...
    for i,
    addHostRequest: = range hypCP.AddHostRequests {
    ...
    for _,
    virtualSwitch: = range hypCP.HypervisorHostProfileTemplate.VirtualSwitches {

        //#########################Virtual switch uplink####################
        virtualSwitchPortUplinks: = make([] map[string] interface {}, 0, len(virtualSwitch.VirtualSwitchUplinks))
        for _,
        virtualSwitchPortUplink: = range virtualSwitch.VirtualSwitchUplinks {
        ...
        for _,
        virtualSwitchPortGroup: = range virtualSwitch.VirtualSwitchPortGroups {
            vspgnetworkUris: = make([] interface {}, len(virtualSwitchPortGroup.NetworkUris))
            for i,
            vspgnetworkURI: = range virtualSwitchPortGroup.NetworkUris {
            ...
            for _,
            virtualSwitchPort: = range virtualSwitchPortGroup.VirtualSwitchPorts {
                    virtualPortPurposes: = make([] interface {}, len(virtualSwitchPort.VirtualPortPurpose))
                    for i,
                    virtualPortPurpose: = range virtualSwitchPort.VirtualPortPurpose {
        ...
        for i,
        networkURI: = range virtualSwitch.NetworkUris {
    ...
    for i,
    sharedStorageVolume: = range hypCP.SharedStorageVolumes {
