...
func main() {
	...
	//fmt.Println(lig)
	//log.Infof("*** get_call => %+v", bytes.NewBuffer(lig))
	...
	for i,up:=range lig_uri.UplinkSets{
		if up.Name=="US-Prod"{
			lig_uri.UplinkSets[i].NetworkUris = append(lig_uri.UplinkSets[i].NetworkUris, nw_set.NetworkUris...)
			fmt.Println(lig_uri.UplinkSets[i].NetworkUris )
	...
	//
	// var q *string=nil
	// lig_uri.EthernetSettings.DomainName=q
	OptionsJSON, _ := json.Marshal(lig_uri.UplinkSets)
	log.Infof("*** liguri => %+v", bytes.NewBuffer(OptionsJSON))
	

	err2 := ovc.UpdateLogicalInterconnectGroup(lig_uri)
	if err2 != nil {
		panic(err2)
	} else {
